...
class Tile(object):
    ...
    def generate_image(self):
        ...
        img = Image.new("RGBA", (Tile.size, Tile.size), (0, 0, 0, 0))
...
class Placemark(object):
    ...
    def move(x, y, top, left, offset):
        """Apply offset"""
        o_x, o_left = divmod(left+offset[1], Tile.size)
        o_y, o_top = divmod(top+offset[0], Tile.size)
        x, y = x + o_x, y + o_y
...
class HotspotsManager(object):
    ...
    def get_base_path(self):
        ...
        return os.path.join(settings.MEDIA_ROOT, self.tiles_dir)
